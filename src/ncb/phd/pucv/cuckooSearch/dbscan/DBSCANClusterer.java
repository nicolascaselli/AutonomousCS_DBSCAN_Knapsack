...
public class DBSCANClusterer<V> {
    ...
    /** lista de puntos no considerados en la clusterizacion */
    private ArrayList<V> noiseValues = null;
    ...
    public DBSCANClusterer(final Collection<V> inputValues, int minNumElements, double maxDistance, DistanceMetric<V> metric) throws DBSCANClusteringException {
        ...
        noiseValues = new ArrayList<V>();
    }
    
    /**
     * Returns the set of points that not consider on any cluster
     * 
     * @return noiseValues an ArrayList of points
     */
    public ArrayList<V> getNoiseValues() {
		return noiseValues;
	}
        ...
        while (inputValues.size() > index) {
            ...
            if (!visitedPoints.contains(p)) {
                ...
                if (neighbours.size() >= minimumNumberOfClusterMembers) {
                    ...
                    for (V value:neighbours) {
                    	if(noiseValues.contains(value))
                    		noiseValues.remove(value);        		
                    }
                }
            }
            index++;
